...
## Environment Variables with Transpiled Packages

When using `transpilePackages` in Next.js to include `@standup/core`, environment variables accessed via `process.env` in the core package will be `undefined` at runtime unless explicitly passed through in `next.config.mjs`.

**Required pattern for Next.js apps:**

```javascript
// apps/web/next.config.mjs
const nextConfig = {
  transpilePackages: ['@standup/core'],
  // Pass env vars needed by @standup/core
  env: {
...
**Why this happens:** When a package is transpiled, the bundler processes it separately and doesn't automatically inherit the runtime environment. The `env` config inlines these values at build time.

**Debugging tip:** If API routes return `undefined` for env vars, check that they're listed in `next.config.mjs`.
