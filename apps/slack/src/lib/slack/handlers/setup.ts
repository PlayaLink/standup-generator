import { getApp, openModal, updateModal } from '../app';
import {
...
export function registerSetupHandler() {
  getApp().command('/standup-setup', async ({ command, ack, client }) => {
    await ack();
  ...
  getApp().action('connect_jira', async ({ ack, body }) => {
    await ack();

    if (body.type !== 'block_actions' || !body.user) return;

    const slackUserId = body.user.id;
    const slackTeamId = body.team?.id || 'unknown';
    const user = await getOrCreateSlackUser(slackUserId, slackTeamId);
  ...
  getApp().action('select_board', async ({ ack, body, client }) => {
    await ack();
  ...
  getApp().action('reconfigure', async ({ ack, body }) => {
    await ack();
