...
BEGIN
  ...
  DROP POLICY IF EXISTS "Service role full access on oauth_tokens" ON oauth_tokens;
  DROP POLICY IF EXISTS "Service role full access on jira_configs" ON jira_configs;
  DROP POLICY IF EXISTS "Service role full access on ticket_names" ON ticket_names;
EXCEPTION
